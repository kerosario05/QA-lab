...
export async function getChecklist(issueKey: string, jobId?: string): Promise<ChecklistResponse> {
...
export async function updateDefectJira(
  issueKey: string,
  defectId: string,
  jiraFields: { jiraIssueKey: string; jiraIssueUrl: string; jiraUploadStatus?: 'uploaded' | 'failed'; jiraUploadError?: string }
): Promise<{ ok: boolean; defect: Defect }> {
  const res = await fetch(`${API_BASE}/api/checklists/${encodeURIComponent(issueKey)}/defects/${encodeURIComponent(defectId)}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
