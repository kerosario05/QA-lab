...
export function TestLaunch({ onLaunch }: TestLaunchProps) {
  ...
  // ── Fetch sections when TR project changes (cache + 60s silent refresh) ──
  useEffect(() => {
    ...
    setSelectedSection(null);
    trSectionsProxy.getSections(projectId, suiteId)
  ...
  }, [step, config.jiraProject, config.status]);
  ...
  const allScenarioKeys = useMemo(
    () => stories.flatMap(st => st.scenarios.map((_, i) => scenarioKey(st.jiraKey, i))),
    [stories],
  );
                    ...
                    <div className="bg-white rounded-xl p-4 border border-[#E8EBEC]">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="text-[24px] font-medium text-[#1a1f2e] mt-0.5" style={{ fontFamily: 'Geist, system-ui, sans-serif', letterSpacing: '-0.03em' }}>{currentTR.suites?.[0]?.id ?? '—'}</div>
                        </div>
                        <div>
                          <div className="text-[10px] uppercase tracking-wider text-[#8B999D]">Test Cases</div>
                          <div className="text-[24px] font-medium text-[#1a1f2e] mt-0.5" style={{ fontFamily: 'Geist, system-ui, sans-serif', letterSpacing: '-0.03em' }}>{currentTR.totalCaseCount}</div>
                        </div>
                      </div>
                    </div>
                  )}
