...
interface Props { issueKey: string; jobId?: string; onBack: () => void; }

export default function DefectChecklist({ issueKey, jobId, onBack }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    getChecklist(issueKey, jobId).then(setData).catch(() => setData(null)).finally(() => setLoading(false));
  }, [issueKey, jobId]);

  const defects: Defect[] = Array.isArray(data?.defects) ? data!.defects : [];
  ...
  // Local fallback: jiraIssueKeys that were created in Jira but failed local PATCH persistence
